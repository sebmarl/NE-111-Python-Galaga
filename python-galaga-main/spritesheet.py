...
class SpriteSheet:
    ...
    def __init__(self, filename):
        ...
        try:
            ...
            self.sheet.set_colorkey(-1, pygame.RLEACCEL)  # pixels with '-1' will be treated as transparent (RG)
    ...
    def image_at(self, rectangle, colorkey=None):
        ...
        image.blit(self.sheet, (0, 0), rect)  # assigns segment of spritesheet onto new surface (RG)
        if colorkey is not None:
            if colorkey == -1:
                colorkey = image.get_at((0, 0))
            image.set_colorkey(colorkey, pygame.RLEACCEL)  # using colourkey parameter to handle transperancy (RG)
        return image
